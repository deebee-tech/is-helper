...
describe('is.date.valid', () => {
  it('should return true for a valid Date', () => {
    expect(is.date.valid(new Date())).toBe(true);
  });

  it('should return true for a Date built from a timestamp', () => {
    expect(is.date.valid(new Date(0))).toBe(true);
  });

  it('should return false for an Invalid Date', () => {
    expect(is.date.valid(new Date('garbage'))).toBe(false);
  });

  it('should return false for a Date built from NaN', () => {
    expect(is.date.valid(new Date(NaN))).toBe(false);
  });

  it('should return false for a date string', () => {
    expect(is.date.valid('2024-01-01')).toBe(false);
  });

  it('should return false for a null', () => {
    expect(is.date.valid(null)).toBe(false);
  });
});

describe('is.date.invalid', () => {
  it('should return true for an Invalid Date', () => {
    expect(is.date.invalid(new Date('garbage'))).toBe(true);
  });

  it('should return false for a valid Date', () => {
    expect(is.date.invalid(new Date())).toBe(false);
  });

  // NOT the same as !valid: a non-Date is neither a valid Date nor an invalid one.
  it('should return false for a non-Date', () => {
    expect(is.date.invalid('not a date')).toBe(false);
  });

  it('should return false for a null', () => {
    expect(is.date.invalid(null)).toBe(false);
  });
});

describe('is.date accepts an Invalid Date', () => {
  /**
   * An Invalid Date IS a Date — it has the internal slot, it just holds NaN. Pinned so nobody
   * "fixes" this into a breaking change. is.date.valid is the check that excludes it.
   */
  it('should return true for an Invalid Date', () => {
    expect(is.date(new Date('garbage'))).toBe(true);
  });
});
