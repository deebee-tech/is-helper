...
/**
 * 4.0.0, 4.1.0 and 4.1.1 all shipped as a bare header with no body -- 4.0.0 was a `feat!:` major
 * whose breaking changes were hidden that way. Nothing failed: semantic-release computed the right
 * version, tagged, and published; only the notes were empty, which no gate looked at.
 *
...
const notesFor = async (type: string, commits: ReturnType<typeof commit>[]) => {
  ...
  return generateNotes(await rngConfig(), {
    cwd: process.cwd(),
    env: process.env,
    options: { repositoryUrl: 'https://github.com/deebee-tech/is-helper' },
